...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.results_table.setHorizontalHeaderLabels([
            ...
            "Total Pages", "Indexed Pages", "External Links Count", "Referring Domains", "Status"
        ])
                    ...
                    if self.results_table.item(row, 5).text() == website:  # Assuming website URL is in column 5
                        ...
                        if result['is_wordpress'] is True:
                            for j, key in enumerate(['domain_rank', 'total_pages', 'indexed_pages',
                                                     'external_links_count', 'referring_domains']):
                        ...
                        self.results_table.setItem(row, 14, QTableWidgetItem(str(result['status'])))
                        break
    ...
    def export_results(self):
        try:
            # Use the last input directory for exports as well
            ...
            if file_name:
                ...
                new_directory = os.path.dirname(os.path.abspath(file_name))
                update_last_input_directory(new_directory)
                logger.info(f"Exporting results to: {file_name}")
                ...
                if success:
                    message = f"Results exported to {file_name}"
                    logger.info(message)
